...
async function handleDeleteGallery(title: string, id: string) {
  if (!id?.length)
    return;
  const isDeleted = await $confirmDelete(title, async () => {
    return blogFetcher().deleteById(id);
  });
  if (isDeleted) {
    emits("onRefresh");
  }
    ...
    >
      <template #head>
        <UiTableRow>
          ...
          <UiTableHead>Caption</UiTableHead>
          ...
          <UiTableRow class="last:[&>td]:!border-0">
            ...
            <UiTableData>
              <div class="flex gap-2">
                <NuxtLink
                  :to="{
                    name: 'dashboard-journey-journeyId',
                    params: {
                      journeyId: item?.id,
                    },
                  }"
                ...
                <button
                  type="button"
                  class="group opacity-80 hover:opacity-100 cursor-pointer text-lg"
                  @click="() => handleDeleteGallery(item?.title, item?.id)"
                >
                  <Icon
                    name="lucide:trash-2"
